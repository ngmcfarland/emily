...
def init_logging(log_file,logging_level,already_started=False):
    ...
    if not already_started:
        logging.basicConfig(filename=log_file, filemode='w', format='%(asctime)s | %(levelname)-8s | %(funcName)-15s | %(message)s', datefmt='%H:%M:%S', level=numeric_level)
    else:
        logging.basicConfig(filename=log_file, format='%(asctime)s | %(levelname)-8s | %(funcName)-15s | %(message)s', datefmt='%H:%M:%S', level=numeric_level)
    return logging
